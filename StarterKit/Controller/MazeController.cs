    ...
    public class MazeController
    {
        ...
        public void Run()
        {
            ...
            else
            {
                ...
                var visitedPositions = new Queue<int[]>();
                List<int[]> correctPath = new();
                List<int[]> shortestPath = new();

                _pathFinder.FindPath(_maze, _maze.Begin, visitedPositions);

                if (_pathFinder is RecursivePathFinder || _pathFinder is StackPathFinder)
                {
                    correctPath = _pathFinder.correctPath;
                    var dijkstra = new DijkstraPathFinder();
                    var dijkstraVisited = new Queue<int[]>();
                    dijkstra.FindPath(_maze, _maze.Begin, dijkstraVisited);
                    shortestPath = dijkstra.correctPath;
                }

                if (_pathFinder is DijkstraPathFinder || _pathFinder is AStarPathFinder)
                {
                    shortestPath = _pathFinder.correctPath;
                }
